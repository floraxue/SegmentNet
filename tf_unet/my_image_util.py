...
class ImageDataProvider(BaseDataProvider):
    ...
    def __init__(self, search_path, a_min=None, a_max=None, data_suffix=".tif", mask_suffix='_mask.tif', shuffle_data=True):
        ...
        self.data_files = self._find_data_files(search_path, shuffle_data)
    ...
    def _find_data_files(self, search_path, shuffle_data=True):
        if not shuffle_data:
            all_files = sorted(glob.glob(search_path))
        else:
            all_files = glob.glob(search_path)
        filtered = [name for name in all_files if self.data_suffix in name and not self.mask_suffix in name]
        return filtered
